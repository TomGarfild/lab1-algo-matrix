...
Matrix inverseMatrixByNewtonsMethod(Matrix A) {
...
void testNewtonsMethod() {
	...
	Matrix A(5);
	A.randomizer();
	Matrix inverseA = inverseMatrixByNewtonsMethod(A);
	cout << "matrix A" << endl;
	cout << A << endl;
	cout << "inverse matrix A by Newtons method" << endl;
	cout << inverseA <<endl;
	cout << "checking by multiply" << endl;
	cout << A*inverseA << endl;

	Matrix B(300);
	std::chrono::steady_clock::time_point Newton_begin = std::chrono::steady_clock::now();
	Matrix inverseB = inverseMatrixByNewtonsMethod(B);
	std::chrono::steady_clock::time_point Newton_end = std::chrono::steady_clock::now();
	...
	for (int i = 0; i < R.rows; i++)
	{
		for (int j = 0; j < R.columns; j++)
		{
			if (i == j)
			{
				if (abs(R.table[i][j] - 1) > 0.000001)
				{
					correct = false;
				}
			}
			else {
				if (abs(R.table[i][j]) > 0.000001)
				{
					correct = false;
				}
			}
	...
	cout << "checking correct = " << correct << endl;

	cout << " " << chrono::duration_cast<std::chrono::milliseconds>(Newton_end - Newton_begin).count() << "ms\n";
	cout << "\n--------- testNewton ---------\n";
}
...
Matrix ordinaryLeastSquares(const Matrix& x, const Matrix& y)
{
	...
	Matrix inverse = inverseMatrixByNewtonsMethod(product);
